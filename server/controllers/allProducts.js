...
const allProducts = async (req, res) => {
    ...
    const maxPriceSchema = z.coerce.number().nonnegative().default(Infinity)
    const categorySchema = z.string().default('');
    const brandSchema = z.string().default('');
    const nameSchema = z.string().default('');
    ...
    const query = {};
    if (minPriceResult !== 0 || maxPriceResult !== Infinity) {
        query.productPrice = {
            $gte: minPriceResult,
            $lte: maxPriceResult
        };
    ...
    if (categoryResult !== '') {
        query.productCategory = categoryResult;
    }
    if (brandResult !== '') {
        query.productBrand = brandResult;
    }
    if (nameResult !== '') {
        query.productName = {
            $regex: nameResult,
